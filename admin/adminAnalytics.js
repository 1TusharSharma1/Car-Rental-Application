...
// 1. Import the role check from the centralized validation file
import { hasRequiredRole } from '../validations/centralizedValidation.js';

document.addEventListener("DOMContentLoaded", () => {
  // 2. Check for admin role using hasRequiredRole
  ...
  // Continue with analytics loading if user is an admin
  loadAnalyticsData();
  loadRecentBookings();
...
function drawBidVsRentalChart() {
  Promise.all([
    ...
    const avgBids = vehicles.map((v) => {
      ...
      return vBids.length > 0
        ? vBids.reduce((sum, b) => sum + b.bid_amount, 0) / vBids.length
        : 0;
    });
  ...
  analyticsUtils.getCachedData("bookings").then((bookings) => {
    const durations = bookings.map((b) => {
      if (b.booking_start_date && b.booking_end_date) {
        return (
          (new Date(b.booking_end_date) - new Date(b.booking_start_date)) /
            (1000 * 60 * 60 * 24) +
          1
        );
      }
  ...
  Promise.all([
    ...
    bookings.forEach((booking) => {
      ...
      if (bid && booking.booking_start_date && bid.bid_date) {
        const diffDays =
          (new Date(booking.booking_start_date) - new Date(bid.bid_date)) /
          (1000 * 60 * 60 * 24);
  ...
  Promise.all([
    ...
    bookings.forEach((booking) => {
      ...
      if (bid && booking.booking_date && bid.bid_date) {
        const diffHours =
          (new Date(booking.booking_date) - new Date(bid.bid_date)) / (1000 * 60 * 60);
      ...
      analyticsUtils.getUserDetails(sellerId).then((user) => {
        ...
        sellerCard.addEventListener("click", () => {
          window.location.href =
            "/seller/analytics/sellerAnalytics.html?sellerId=" +
