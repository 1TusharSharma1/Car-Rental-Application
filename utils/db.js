...
function openDB(callback) {
  ...
  request.onupgradeneeded = function (event) {
    ...
    if (!db.objectStoreNames.contains("categories")) {
      const catStore = db.createObjectStore("categories", { keyPath: "category_id" });
      catStore.createIndex("supercategory_id", "supercategory_id", { unique: false });
    }
    ...
    if (!db.objectStoreNames.contains("vehicles")) {
      const vehiclesStore = db.createObjectStore("vehicles", { keyPath: "vehicle_id" });
      vehiclesStore.createIndex("vehicle_owner_id", "vehicle_owner_id", { unique: false });
      vehiclesStore.createIndex("category_id", "category_id", { unique: false });
      vehiclesStore.createIndex("location", "location", { unique: false });
    }

    // 6) BOOKINGS
    if (!db.objectStoreNames.contains("bookings")) {
      const bookingsStore = db.createObjectStore("bookings", { keyPath: "booking_id" });
